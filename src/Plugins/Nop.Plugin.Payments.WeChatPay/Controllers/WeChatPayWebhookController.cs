...
{
    public class WeChatPayWebhookController : Controller
    {
        #region Fields
        ...
        public WeChatPayWebhookController(ILogger logger,
        ...
        public async Task<IActionResult> SecurePayWebhook(SecurePayWebhookRequest request)
        {
            try
            {
                ...
                if (plugin is not WeChatPayPaymentProcessor processor || !_paymentPluginManager.IsPluginActive(processor))
                ...
                var verifySign = CommonHelpers.GenerateSign(_weChatPayPaymentSettings.ApiToken, signingParams);
