...
using Senparc.Weixin.CommonService.CustomMessageHandler;
using Senparc.Weixin.CommonService.MessageHandlers;
...
using Senparc.Weixin.CommonService.MessageHandlers.Middleware;  //替换下面3个Middleware的引用
...
{
    public static void UseNopSenparcWeixin(this IApplicationBuilder application)
    {
        ...
        //application.UseEnableRequestRewind();
        ...
        var registerService = application.UseSenparcGlobal(webHostEnvironment, senparcSetting, globalRegister =>
        {
            #region CO2NET 全局配置
            ...
            //配置Memcached缓存（按需，独立）
            if (UseMemcached(senparcSetting, out var memcachedConfigurationStr)) //这里为了方便不同环境的开发者进行配置，做成了判断的方式，实际开发环境一般是确定的，这里的if条件可以忽略
            {
            ...
            .UseSenparcWeixin(senparcWeixinSetting, (weixinRegister, weixinSetting) =>
            {
                ...
                */
                ...
                if (UseMemcached(senparcSetting, out _))
                {
                    application.UseEnyimMemcached();
                ...
                weixinRegister
                        ...
                        //.RegisterWxOpenAccount(senparcWeixinSetting.Items["第二个小程序"], "第二个【盛派网络小助手】小程序")
                ...
                #region 注册企业号（按需）
                        ...
                        //注册企业微信（可注册多个）
                        //.RegisterWorkAccount(senparcWeixinSetting, "企业微信")
                        // .RegisterWorkAccount(senparcWeixinSetting["企业微信审批"], "企业微信审批")
                ...
                #region 注册微信支付（按需）
                        ...
                        //注册旧微信支付版本（V2）（可注册多个）
                        //.RegisterTenpayOld(senparcWeixinSetting, "公众号")//这里的 name 和第一个 RegisterMpAccount() 中的一致，会被记录到同一个 SenparcWeixinSettingItem 对象中

                        //注册最新微信支付版本（V3）（可注册多个）
                        .RegisterTenpayV3(senparcWeixinSetting, "公众号")//记录到同一个 SenparcWeixinSettingItem 对象中
                    /* 特别注意：
                    ...
                    .RegisterTenpayApiV3(senparcWeixinSetting, "公众号-ApiV3")//注册最新的 TenPay V3
                ...
                #region 注册微信第三方平台（按需）
                    ...
                    //        var dir = Path.Combine(fileProvider.MapPath("~/App_Data/OpenTicket"));
                    ...
                    //    {
                    //        var dir = Path.Combine(fileProvider.MapPath("~/App_Data/AuthorizerInfo/" + componentAppId));
                    //        if (!Directory.Exists(dir))
                    //            Directory.CreateDirectory(dir);
                ...
                #region 设置自定义 ApiHandlerWapper 参数（可选，一般不需要设置）
        ...
        application.UseMessageHandlerForMp("/WeixinAsync", CustomMessageHandler.CustomMessageHandler.GenerateMessageHandler, options =>
        {
            /* 说明：
        ...
        //application.UseMessageHandlerForWxOpen("/WxOpenAsync", CustomWxOpenMessageHandler.GenerateMessageHandler, options =>
        //{
        //    options.DefaultMessageHandlerAsyncEvent = DefaultMessageHandlerAsyncEvent.SelfSynicMethod;
        ...
        //application.UseMessageHandlerForWork("/WorkAsync", WorkCustomMessageHandler.GenerateMessageHandler,
        //                             o => o.AccountSettingFunc = c => senparcWeixinSetting);//最简化的方式
    ...
    /// 配置微信跟踪日志（演示，按需）
    /// </summary>
    private static async void ConfigTraceLog()
    {
