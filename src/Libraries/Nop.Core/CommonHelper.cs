...
namespace Nop.Core
{
    ...
    public partial class CommonHelper
    {
        ...
        public static string GenerateRandomDigitLetterCode(int length, bool digit = true, bool letter = true)
        {
            ...
            {
                if (digit && !letter) //仅数字
                    str = string.Concat(str, pattern[random.Next(0, 10)]);
                else if (!digit && letter) //仅字母
                    str = string.Concat(str, pattern[random.Next(10, 36)]);
                else //数字和字母
                    str = string.Concat(str, pattern[random.Next(0, 36)]);
            }
            return str;
        }
